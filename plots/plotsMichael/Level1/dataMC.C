...
TString outdir = "~/www/php-plots/2LeptonStops13TeV/dataMC/";
...
void DataMCplots(TString plotname){
	  ...
	  gStyle->SetMarkerColor(37);
          ...
          TFile data1("ntuples/DoubleElec-PromptReco-v1/output/output.root");
          TFile data2("ntuples/DoubleMuon-PromptReco-v1/output/output.root"); 
       //   TFile data3(indir+"/Muon_Elec/output/Muon_Elec_"+selectionname+"_"+leptonname+".root"); 
          TFile ttbar("ntuples/TTJets/output/output.root"); 
          TFile wjets("ntuples/WJetsToLNu/output/output.root"); 
          TFile others("ntuples/others.root");
          TFile dy("ntuples/dy.root");
          TFile qcd("ntuples/qcd.root");
...
//		  TH1D* h3= (TH1D*)data3.Get(plotname);
	  ...
	  TH1D* TTBar= (TH1D*)ttbar.Get(plotname);
	  TH1D* QCD= (TH1D*)qcd.Get(plotname);
	  TH1D* DY= (TH1D*)dy.Get(plotname);
	  ...
	  h_mc->Add(DY);
          ...
          */ 
	      ...
	      binunits = "GeV";	 
              ...
              else if(plotname == "h1_nlept"){
                data_xaxis->SetTitle("Lepton Multiplicity");
              }
              else if(plotname == "h1_nbjets"){
                data_xaxis->SetTitle("b-jet Multiplicity");
              }
              else if(plotname == "h1_dR_LepJet"){
                data_xaxis->SetTitle("#DeltaR(lepton_{1}, jet_{1})");
              }
              else if(plotname == "h1_nvTX"){
                data_xaxis->SetTitle("Number of vertices");
              }
           ...
           legge = leg->AddEntry(QCD,   "QCD", "F");
           legge = leg->AddEntry(TTBar,   "t#bar{t}", "F");
           legge = leg->AddEntry(WJets,   "W + Jets", "F");
           legge = leg->AddEntry(DY,   "Drell-Yan", "F");
           ...
           l1.SetTextSize(0.05);
           l1.SetNDC();
           l1.DrawLatex(0.18, 0.98, "CMS preliminary, L = 42 pb^{-1}");
...
void makeAllPlots(){
	...
	DataMCplots("h1_lepton_eta");
	DataMCplots("h1_njets");
	DataMCplots("h1_nbjets");
	DataMCplots("h1_nlept");
	DataMCplots("h1_nvTX");
